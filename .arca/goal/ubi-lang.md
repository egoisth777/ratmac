...
| Term | Definition |
|---|---|
| Machine | The state machine as a whole — States, transitions, Exit Guards — pure data declared by a Machine Class, never run by agents; an agent sees only its State Prompt, never the graph (ADR-0009). |
| Machine Class | The state-machine definition in the tracked `ratmac.toml` of the invoking checkout. Data, not code. A template: declares States, transitions, Exit Guards. Human-written and reviewed, never agent-authored. |
...
| Run | A live instance of a Machine Class, created by `rtm start` (class vs instantiation, ADR-0005). The whole live instance: each Run owns its Run Record and run-local artifacts; the Engine transition log is shared. |
| Scheduler | The generic engine — Rust CLI, binary `rtm`. Sole writer of Run Records. Holds no project-specific knowledge. |
| State | Where a Run sits in its machine graph: a node of the Machine where agent work happens. The ONLY dimension of machine position (ADR-0001, ADR-0012). Always say "State"; never "Phase", and never "state" for the file or the Run (SVC-001). |
| Status | State-local lifecycle (`planned\|executing\|blocked\|passed\|failed`) recorded by the Scheduler. Not part of the Machine graph and never a position in it (ADR-0001, SVC-001). |
| Exit Guard | A predicate over the working tree, evaluated by the Scheduler at `rtm step`. Checks artifacts, never agent claims; passing ALL of a State's Exit Guards is the only way to leave it. The closed guard-kind vocabulary — each kind's semantics, required fields, and forbidden fields — is defined once in `.arca/runbook-spec.md` and restated nowhere else (RBS-002, RBS-004). |
| Run Record | `runs/<run-id>/run.toml` under the Engine root: the one file the Engine writes for one Run, holding its current `state`, `status`, and run-scoped fields. Written ONLY by the Scheduler; all agents read, never write (ADR-0003, ADR-0012). |
| Pre-cutover position spelling | `Phase`, a runbook `phases` table, the `state.toml` filename, and a Run Record `phase` field: the superseded spellings of State and Run Record. They survive only as unmodified history — archived issue bundles, archived tickets, archived gap records, and `.arca/log.md` — enumerated in the live-surface audit's allowlist, and never appear on a live surface (SVC-008). |
| Pre-cutover machine residue | A live runbook declaring `phases`, or a live Run Record at the pre-cutover filename or carrying the pre-cutover position field. The addressed entry point refuses and instructs, naming the artifact and the repair, before any read, join, parse, or write; nothing is migrated in place (SVC-005). |
...
| State Prompt | What an agent receives for a State: inline prose from `ratmac.toml` + the Scheduler-generated Exit Guard and legal-input list (ADR-0009). The ONLY machine information ever shown to an agent. |
...
| `rtm status` | Read-only report of a Run's State, Status, pending guards, and resolved Engine root. |
...
| Contract gate | A mechanized State gate that verifies that State's artifact contract and evidence, so status or prose edits alone can never satisfy it. |
...
| Runbook specification | `.arca/runbook-spec.md`: the single written authority for the Machine Class format — top-level shape, State and transition fields, the guard-kind vocabulary, ownership rules, and the diagnostic-code table. Code implements it; nothing restates it. |
...
| Legal transition input list | The closed set of exact values a branching State declares under `inputs`; complete unique coverage means each value labels one ordinary outgoing transition and every such transition carries one listed value. |
| Transition input | The single value selected by external evidence review for one addressed Run at one current State. It selects an ordinary transition but never bypasses readiness guards. |
...
| Straight-line State | A State with exactly one ordinary outgoing transition. It declares no legal input list, its edge has no input label, and movement needs no verdict. |
| Live verdict record | The strict `verdict.toml` currently published in one Run's Verdict slot: `state`, `input`, and `rationale`. It is absent when the slot is empty. |
...
| Terminal State | A State with no ordinary outgoing edge. Entering it completes ordinary execution. |
| Passed Run | A Run whose Engine-owned status is `passed` because it started in or advanced into a Terminal State. |
| Abandoned event | The durable Engine-written history fact recorded before an explicitly abandoned Run's active state is retired. `abandoned` is not a surviving Run Record value. |
| Failed outcome | A deferred terminal outcome with no current Engine-observable trigger. Guard refusal is not failure. |
| Spawn | Ordinary checked motion creating a child Run from a class the parent's runbook declares; legal only while the parent occupies the spawning State; no confirmation phrase. |
...
| Join | The guard that reads each non-abandoned ledger child's Engine-written terminal fact and passes only when every such child stands at a graph-terminal State with status `passed` and the satisfied count meets the declared minimum. A refusal names every non-satisfying child. |
| Child-as-reviewer | The first-increment judge-independence mechanism: a spawned child machine produces the judgment a parent's branching State consumes. |
