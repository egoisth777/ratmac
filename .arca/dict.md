...
- **Triage class**: The cause a red, unexpired sweep verdict is sorted into before anything is done to the lane - retired contract, fixture drift, or live regression. The class, not the failure message, decides the act (schema.md, "Landed-lane recovery").
- **Retired contract**: A red verdict whose lane asserts an interface, path, record key, or wording a later authorized landing replaced on purpose; the behaviour the lane was cut to prove still exists under the new spelling. Ported, naming the landing.
- **Fixture drift**: A red verdict whose lane's own scaffold no longer builds what the Engine accepts - an obsolete layout, a citation taken before the Engine mints it - while the behaviour under test and the Engine's answer are unchanged. Ported.
