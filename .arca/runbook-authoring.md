...
| `RB111` | The runbook still declares the pre-cutover [`phases`](runbook-spec.md#states) table. | Rename it to [`states`](runbook-spec.md#states) (and any `[[phases.<name>.spawns]]` to `[[states.<name>.spawns]]`, `[classes.<name>.phases]` to `[classes.<name>.states]`); nothing is migrated automatically. | — |
