...
- `.arca/issue/i-015-cycle-as-runbook/design.md:126` — "No agent-spawning, no process management, no scheduling: the Run is still stepped by a caller."
...
6. `.arca/goal/design.md:88` (ADR-0010) and `i-015/design.md:126` — no process management, no spawn flag, no scheduling.
...
6. `i-015/design.md:18-20` — "Run per ticket" is recorded as a live option with no rejection marker, while (a) and (d) carry one.
