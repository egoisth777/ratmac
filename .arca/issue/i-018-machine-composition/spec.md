...
| :--- | :--- | :--- | :--- | :--- |
| `FDC-007` | Authorization splits by motion kind: `spawn` is ordinary motion with no confirmation phrase; `respawn` and abandon-with-run-id require confirmation phrases naming the run id. | pending | Billy selected the deferred bundle on 2026-08-03: routing, delivery, and completion have landed, so the authorization split is buildable checked motion. Provenance: adopted default (`08` §4 item 15, closing `AR-11`; batch human sign-off 2026-07-29; revocable). | — (pending) |
| `FDC-008` | Every Phase on a cycle carries at least one outgoing edge guarded by receipt- or contract-class guards. | pending | Termination becomes a static check by guard-kind membership; the closed guard vocabulary and doctor lanes it needs are integrated. Provenance: adopted default (`08` §4 item 13, closing `AR-07`; revocable). | — (pending) |
| `FDC-009` | The runbook format carries class and spawn tables, with `blocked-route` as the canonical spelling. | pending | The format single source of truth, typed parser, and doctor exist to grow the tables; the prior format content that would refuse them is superseded at integration while `RBS-004`'s single-authority rule is untouched. Provenance: individual human ruling (`AR-10`, 2026-07-29). | — (pending) |
| `FDC-010` | Judge independence lands child-as-reviewer first; the witnessed verdict verb remains deferred. | pending | Child-as-reviewer needs only spawn/join mechanics; signer identity stays outside the Engine (`ORS-001`), so the witnessed verdict verb stays deferred — recorded, not dropped. Provenance: adopted default (`08` §4 item 17; revocable). | — (pending) |
