...
This bundle sits in the intake work area; the dispositions below are the
DESIGNER's proposal, and the next planning pass (P1) confirms or revises
them. Three boundaries are proposed as open for P1 rather than decided here,
because the wish names them as human choices: the command boundary (one
entry point with open/close/status verbs versus separate commands), where
the lifecycle lives (a repo-local tool beside
[tools/trial.ps1](../../../tools/trial.ps1), a runbook-declared lifecycle
the Engine drives, or an Engine subcommand), and who owns recovery when an
interrupted close cannot resume cleanly.
