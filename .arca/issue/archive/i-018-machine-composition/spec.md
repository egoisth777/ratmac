...
| :--- | :--- | :--- | :--- | :--- |
| `FDC-007` | Authorization splits by motion kind: `spawn` is ordinary motion with no confirmation phrase; `respawn` and abandon-with-run-id require confirmation phrases naming the run id. | accepted | Billy selected the deferred bundle on 2026-08-03: routing, delivery, and completion have landed, so the authorization split is buildable checked motion. Provenance: adopted default (`08` §4 item 15, closing `AR-11`; batch human sign-off 2026-07-29; revocable). | [goal FDC-007](../../../goal/spec.md#integrated-machine-composition-requirements) |
| `FDC-008` | Every Phase on a cycle carries at least one outgoing edge guarded by receipt- or contract-class guards. | accepted | Termination becomes a static check by guard-kind membership; the closed guard vocabulary and doctor lanes it needs are integrated. Provenance: adopted default (`08` §4 item 13, closing `AR-07`; revocable). | [goal FDC-008](../../../goal/spec.md#integrated-machine-composition-requirements) |
| `FDC-009` | The runbook format carries class and spawn tables, with `blocked-route` as the canonical spelling. | accepted | The format single source of truth, typed parser, and doctor exist to grow the tables; the prior format content that would refuse them is superseded at integration while `RBS-004`'s single-authority rule is untouched. Provenance: individual human ruling (`AR-10`, 2026-07-29). | [goal FDC-009](../../../goal/spec.md#integrated-machine-composition-requirements) |
| `FDC-010` | Judge independence lands child-as-reviewer first; the witnessed verdict verb remains deferred. | accepted | Child-as-reviewer needs only spawn/join mechanics; signer identity stays outside the Engine (`ORS-001`), so the witnessed verdict verb stays deferred — recorded, not dropped. Provenance: adopted default (`08` §4 item 17; revocable). | [goal FDC-010](../../../goal/spec.md#integrated-machine-composition-requirements) |
| `FDC-011` | The spawn-ledger contract, at the per-run path `FDC-004` reserves under the parent Run's directory: Scheduler-owned, append/annotate-only — agents never write it. `rtm spawn` appends one entry carrying the child run id, child class, binding values, the git revision at spawn, and the child workspace path when one is created. Human-confirmed abandon flips only that entry's abandoned mark; human-confirmed respawn appends the successor entry recording the superseded id (`FDC-006`'s remainder). The ledger fixes the join's expected set: an entry whose child Run is missing on disk refuses loudly, never silently shrinks the set. | accepted | Home ruled 2026-08-03 (Billy): this spec carries the contract, extending the 2026-07-29 scope settlement (accepted review fix) that reserved the path in run residency and named this issue the contract's home. Contents condensed from the research's spawn-ledger section ([05-invocation-join.md](../../../research/re-ratmac-FSM/05-invocation-join.md), "The spawn ledger"). | [goal FDC-011](../../../goal/spec.md#integrated-machine-composition-requirements) |
| `FDC-012` | Composition is capped at one level: a spawned child Run may not itself spawn, and the Engine refuses a spawn addressed to a Run recorded as a child in any spawn ledger, naming the cap. | accepted | Individual human ruling, 2026-08-03 (Billy), resolving this issue's recursion-depth fork: capped until need shows; the cap is checkable at the spawn boundary and lifting it later is additive. | [goal FDC-012](../../../goal/spec.md#integrated-machine-composition-requirements) |
...
- Every requirement above names its provenance. `FDC-007`–`FDC-010` trace to their decision records in the
  seed's [design.md](../i-016-fsm-doctrine-convergence/design.md): human ruling or adopted
