...
fn copy_fixture_to_temp() -> PathBuf {
    let nonce = SystemTime::now()
        ...
        .as_nanos();
    let root = std::env::temp_dir().join(format!("arca-scheduler-t015-{nonce}"));
...
fn corrupt_state_halts_without_guessing() {
    ...
    let status = Scheduler::open(&root).expect("valid class should open");
    let status_error = status
        .status()
        .expect_err("status must reject malformed state.toml");
    assert_actionable_parse_error(status_error);

    let reader = Scheduler::open(&root).expect("valid class should open");
